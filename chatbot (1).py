...
class RoutineChatbot:
    ...
    def _send_pending_email(self) -> str:
        ...
        if self.google is None or self.google.creds is None:
            return (
                f"I have the email ready for {pending['to_email']}, but Gmail isn't connected "
                "(add credentials.json / token.json and enable gmail). Nothing was sent."
            )
        try:
            self.google.send_email(pending["to_email"], pending["subject"], pending["body"])
            return f"Sent. The email to {pending['to_email']} (\"{pending['subject']}\") is on its way."
    ...
    def _handle_email(self, text: str) -> str:
        ...
        if self.pending_email and any(word in lower for word in ["confirm", "yes", "approve", "ok", "send it", "send"]):
            return self._send_pending_email()

        if self.pending_email and any(word in lower for word in ["cancel", "no", "reject"]):
            self.pending_email = None
        ...
        extracted = self.email_agent.extract_email_intent(text)
        if extracted and extracted.get("to_email") and validate_email_address(extracted["to_email"]):
            subject = extracted["subject"]
            body = extracted["body"] or self.email_agent.draft_email(email, subject, text)
        else:
            subject = "Meeting request"
            body = self.email_agent.draft_email(
                email,
                subject,
                "I am reaching out to ask for a good time to connect. Please let me know when you are free.",
            )

        self.pending_email = {"to_email": email, "subject": subject, "body": body}
        return (
            f"Here's the draft for {email}:\n\n"
            f"Subject: {subject}\n\n{body}\n\n"
            "Reply 'confirm' to send it, or 'cancel' to drop it."
        )
